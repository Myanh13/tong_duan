...
const Ql_nv = () => {
  ...
  const fetchUsers = async () => {
    ...
    const url = query.trim()
                ? `https://tong-api-1.onrender.com/search_nhanvien?query=${query}`
                : 'https://tong-api-1.onrender.com/admin/nhanvien/role'; // Nếu không có từ khóa, gọi API lấy tất cả sản phẩm
   ...
   useEffect(() => {
          fetchUsers(); // Gọi hàm fetch data khi component được mount hoặc khi query thay đổi
      }, [query]);
  ...
  return (
    <div className="admin_table_pra_wrapper">
      ...
      <h2 className="admin_table_pra_title">{query ? 'Kết quả tìm kiếm' : 'Quản lý Nhân Viên'}</h2>
      ...
      <table className="admin_table_pra">
        ...
        {query ? (
            searchResults.length > 0 ? (
              ...
              <tr>
                  <td colSpan="7">   không có kết quả nào trùng khớp </td>
              </tr>
          )
              ) : (
